...
| `parcel.get_status.v1` / `parcel_get_status` | Required `MGB-` project tracking ID; returns normalized status, event time, and coarse location. | `mock` in pilot; authenticated partner/sandbox candidate later. All identifiers are project-local and non-real. | MNG developer portal: https://apizone.mngkargo.com.tr/en/node/3973. No real customer name, address, phone, shipment creation, rerouting, or cancellation. |
...
- Parcel tracking: sandbox/partner access and project-only identifier policy.
