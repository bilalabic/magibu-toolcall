...
| Benchmark leakage | Invalid evaluation | separate paths and hash/fingerprint hooks | Access controls and semantic scan later |
...
| Turkish quality cannot be deterministic | Unnatural records | language-review role and semantic hook | Native reviewer approval |
| Self-approval or insufficient review | Quality/governance failure | contributor identity and reviewer-role checks | Staff reviewers and sampling plan |
...
| Application policy mistaken for filesystem security | benchmark leakage | CLI permission/scope checks and audit events | Infrastructure owner applies OS/object-store ACLs separately |
