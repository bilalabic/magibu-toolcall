...
| 12 | Review and export (`review/`) | GitHub PR lifecycle marker and accepted-only export tests pass |
...
| 18 | Scalable jobs (`batch.py`, job schema, CLI) | Benchmark isolation, shard/checkpoint/resume, collision, and failure-queue tests pass |
| 19 | Production CLI and handover (`cli.py`, Turkish README files, architecture/traceability docs) | Source import/localization, batch plan/status, semantic provider selection, GitHub PR review conventions, export/freeze/run, full suite, and deterministic end-to-end tests pass |
...
| 21 | Evidence-backed dataset quality and PR-ready lifecycle (`quality.py`, review schema/CLI) | Declared-mode execution, result comparison, deterministic/semantic duplicate gates, durable reports, automatic-gate protection, and full-suite tests pass |
...
manifest, provenance, validation, quality, or GitHub PR review gates. Translation
...
Human approval and discussion are persisted by GitHub PR; the record keeps only
its accepted/needs-revision/rejected lifecycle state.
