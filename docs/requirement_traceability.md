...
| DeepSeek/OpenAI role boundaries | `generation/providers.py`, `network.py`, `semantic.py`, config | structured JSON, retry, secret-safe errors, embeddings/cache tests | implemented-production-transport; semantic response judge remains interface |
...
| Semantic decision evaluation | `generation/providers.py` protocol | deterministic test-double tests | implemented-interface; OpenAI judging deferred-production |
...
| Resumable bulk Turkish scenario generation | `batch.py`, generation providers, dataset/benchmark CLI | shard, checkpoint, retry, failure queue, collision, and resume tests | implemented-infrastructure; production run/spend deferred |
...
| Evidence-backed dataset quality | `quality.py`, `dataset quality`, review decision workflow | local execution/result comparison, exact and semantic duplicate states, test-double non-certification, reports, computed reviewer acceptance | implemented; live OpenAI/API evidence and real reviewer decisions pending |
...
- Partially implemented by deliberate boundary: free-text semantic/Turkish
...
- Blocked on external decisions: approved tool list, licenses/API access,
  provider model/version/threshold choices, reviewer identities, storage ACLs,
  and budgets.
